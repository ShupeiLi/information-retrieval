"""
Group 15, Task 4
A modified version of CERerankingEvaluator*: Evaluation metric from MRR@10 to NDCG@10.
...
class CERerankingEvaluator:
    ...
    documents and sorts them in decreasing order. Then, NDCG@10 is computed to measure the quality of the ranking.
    ...
    def __init__(self, samples, ndcg_at_k: int = 10, name: str = '', write_csv: bool = True):
        ...
        self.csv_headers = ["epoch", "steps", "NDCG@{}".format(ndcg_at_k)]
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        for instance in self.samples:
            ...
            all_ndcg_scores.append(ndcg_score(np.asarray([is_relevant]), np.expand_dims(pred_scores, axis=0), k=10))

        mean_ndcg = np.mean(all_ndcg_scores)
        logger.info(
            "Queries: {} \t Positives: Min {:.1f}, Mean {:.1f}, Max {:.1f} \t Negatives: Min {:.1f}, Mean {:.1f}, "
            "Max {:.1f}".format(
        ...
        logger.info("NCDG@{}: {:.2f}".format(self.ndcg_at_k, mean_ndcg * 100))
        ...
        if output_path is not None and self.write_csv:
            ...
            with open(csv_path, mode="a" if output_file_exists else 'w', encoding="utf-8") as f:
                ...
                writer.writerow([epoch, steps, mean_ndcg])
